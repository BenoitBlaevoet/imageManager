(() => {
        ...
        card.innerHTML = `
            ...
            <div class="component-card-body">
                ...
                <div class="component-card-actions">
                    ...
                    ${hasImage ? `<a href="editor?component_id=${esc(comp.id)}&image_id=${esc(comp.active_image.id)}" class="btn btn-primary btn-sm">Edit Crops</a>` : ''}
    ...
    async function loadPickerTagFilter() {
        const tags = ImageLib.getTags().length
            ? ImageLib.getTags()
            : (await (await fetch('api/tags')).json()).tags || [];
    ...
    async function loadPickerImages() {
        ...
        images.forEach(img => {
            ...
            card.querySelector('.btn-picker-add').addEventListener('click', async () => {
                await fetch('api/component-images', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    async function loadImageBrowser(componentId) {
        ...
        const res  = await fetch(`api/component-images?component_id=${encodeURIComponent(componentId)}`);
        const data = await res.json();
        ...
        data.images.forEach(img => {
            ...
            card.querySelector('.btn-remove-img').addEventListener('click', async () => {
                ...
                await fetch(`api/component-images?id=${img.assignment_id}`, { method: 'DELETE' });
                loadImageBrowser(componentId);
                loadComponents();
            });
    ...
    async function openComponentsModal() {
        ...
        const [cRes, pRes] = await Promise.all([fetch('api/components'), fetch('api/presets')]);
